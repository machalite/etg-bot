def save_gun(conn, list_gun, coll_name):
    ...
    for item in list_gun:
            cursor = conn[coll_name].find(
                {'name': item.get('name'), "class": "Gun"})
            if cursor.count() == 0:
...
def save_gundead(conn, list_gundead, coll_name):
    print("Saving gundeads...")
    count_ins = 0
    count_upd = 0
    for item in list_gundead:
            cursor = conn[coll_name].find(
                {'name': item.get('name'), "class": "Gundead"})
            if cursor.count() == 0:
                conn[coll_name].insert_one(item)
                count_ins += 1
            else:
